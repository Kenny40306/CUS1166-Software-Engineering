...
public class MainControllerFrame extends JFrame{
    ...
    private JPanel createJobRow(VCController.JobRequest req) {
       ...
       //Buttons 
        JButton approve = UIStyling.createIconButton("\u2714", approveColor, hoverApproveColor); // A or "✓" or \u2714
        JButton reject  = UIStyling.createIconButton("\u2716", rejectColor, hoverRejectColor); // R or "✕" or \u2715

        Font symbolFont = new Font(Font.DIALOG, Font.BOLD, 11); //cross-platform safe for mac os and windows
        approve.setFont(symbolFont);
        reject.setFont(symbolFont);
        
        if (!approve.getFont().canDisplay('\u2714')) {
            approve.setText("Accept");
        }

        if (!reject.getFont().canDisplay('\u2716')) {
            reject.setText("Reject");
        }
        
        //Button Action Listeners
        approve.addActionListener(e -> {
            vcController.approveJob(req.requestID);
            refreshRequests();
        });

        reject.addActionListener(e -> {
            vcController.rejectJob(req.requestID);
            refreshRequests();
        });
    ...
    private JPanel createVehicleRow(VCController.VehicleRequest req) {
     ...
     // ================= MULTI-LINE VEHICLE INFO =================
        ...
        //Button Action Listeners
        approve.addActionListener(e -> {
            vcController.approveVehicle(req.requestID);
            refreshRequests();
        });

        reject.addActionListener(e -> {
            vcController.rejectVehicle(req.requestID);
            refreshRequests();
        });
    ...
    //Old Version Note: Real data source is memory where MainControllerFrame UI is still reading from memory like java object values not SQL database state directly
    //UI edits VCController memory then VCController updates SQL after admin action of approval for permanent data storage
    ...
    //Overall Hybrid Architecture- Memory driven with Database-backed persistence
    //Memory handles pending request, fifo scheduling and real-time GUI updates
    //Database SQL handles permanent storage, approved data retrieval, persistent edits and fifo locking
